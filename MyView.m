...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        colors= [@[[UIColor blackColor], [UIColor greenColor], [UIColor blueColor], [UIColor redColor], [UIColor blackColor], [UIColor whiteColor]] mutableCopy];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([lineArray count]>0) {
        ...
        if ([array count]>0)
        {
            ...
            {
                CGPoint myEndPoint=CGPointFromString(array[j + 1]);
                //--------------------------------------------------------
                CGContextAddLineToPoint(context, myEndPoint.x,myEndPoint.y);
            }
            //获取colorArray数组里的要绘制线条的颜色
            NSNumber *num= colorArray[i];
            int count=[num intValue];
            UIColor *lineColor= colors[count];
            //获取WidthArray数组里的要绘制线条的宽度
            NSNumber *wid= WidthArray[i];
    ...
    {
        CGContextBeginPath(context);
        CGPoint myStartPoint=CGPointFromString(pointArray[0]);
        CGContextMoveToPoint(context, myStartPoint.x, myStartPoint.y);
        ...
        UIColor *lineColor= colors[colorCount];
